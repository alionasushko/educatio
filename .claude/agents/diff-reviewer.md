...
- **Boundary discipline** — `apps/web` must not import `mongoose`/`mongodb`, `@liveblocks/node`, `@ai-sdk/google`, `ai`, `@vercel/blob`, `resend`, `bcrypt`, plus `next-auth`/`@auth/mongodb-adapter`, which are not installed and are guarded pre-emptively. ESLint's `no-restricted-imports` catches direct imports; flag anything indirect it might miss (re-exports, dynamic imports).
