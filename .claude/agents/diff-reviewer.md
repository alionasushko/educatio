...
- **Boundary discipline** — `apps/web` must not import `mongoose`/`mongodb`, `@liveblocks/node`, `@ai-sdk/google`, `ai`, `@vercel/blob`, `resend`, `next-auth`, `@auth/mongodb-adapter`. ESLint's `no-restricted-imports` catches direct imports; flag anything indirect it might miss (re-exports, dynamic imports).
