...
> **Status.** Every screen in `docs/SPEC.md` §Features is built, and every api endpoint is implemented. The web→api request layer is finished and hardened — one `server-only` seam, a response schema required on every call, one `ActionResult` shape for every Server Action, and route boundaries for errors and 404s (see `docs/ARCHITECTURE.md` §Request layer for the rules and the rejected alternatives). The remaining work is **deployment** — Vercel, a domain, `SENTRY_DSN`, and transactional email, without which a deployed sign-up answers `503 service_unavailable` and only the flag-gated demo account can get in — plus a Lighthouse pass and wider test coverage.
>
> **Verification status.** Everything compiles, builds, and passes `npm run check`. The api's responses are pinned to the shared contract by `apps/api/test/api-contract.spec.ts`. `apps/web` now has Vitest (75 tests) and Playwright (51 tests, chromium + webkit) suites covering the canvas, the join flow, the summary, account settings and session-cookie lifetimes against **live Liveblocks**, live Vercel Blob and local Mongo. Still thin: the request layer and `proxy.ts`'s kind gate are only covered incidentally. The magic-link sign-up / sign-in / verify / demo flows have been run against **local** Mongo (Resend unconfigured in dev, so the magic link is logged to the api console). Email + password sign-in has not been run in a browser, and nothing has run against live Resend or Gemini — treat those as unverified.
