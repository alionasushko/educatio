...
> **Status.** Every screen in `docs/SPEC.md` §Features is built, and every api endpoint is implemented. The web→api request layer is finished and hardened — one `server-only` seam, a response schema required on every call, one `ActionResult` shape for every Server Action, and route boundaries for errors and 404s (see `docs/ARCHITECTURE.md` §Request layer for the rules and the rejected alternatives). The remaining work is **deployment** — Vercel for web, Google Cloud Run for api (see `docs/ARCHITECTURE.md` §Deployment for the flags, the `TRUST_PROXY` value and what was rejected) — plus a Lighthouse pass and wider test coverage. A domain, `SENTRY_DSN` and transactional email are optional rather than blocking: `ENABLE_DEMO_LOGIN` is the entry path, so a visitor needs no mailbox, and without `RESEND_API_KEY`/`EMAIL_FROM` a real sign-up still answers `503 service_unavailable` by design. A whole-app security review ran before deploy; confirmed findings are fixed and the deliberate deferrals, each with the condition that reopens it, are in `docs/SECURITY.md`.
>
> **Verification status.** Everything compiles, builds, and passes `npm run check`. The api's responses are pinned to the shared contract by `apps/api/test/api-contract.spec.ts`. `apps/web` has Vitest (112 tests) and Playwright (57 tests across 7 specs, chromium + webkit) suites covering the canvas, the join flow, the summary, account settings and session-cookie lifetimes against **live Liveblocks**, live Vercel Blob and local Mongo. `proxy.ts`'s kind gate is now covered directly by `apps/web/src/__tests__/proxy.test.ts` — forged and expired tokens, a student turned back from every path that is not their own room, and an encoded-id bypass attempt. `api-client.ts` is covered too — the response-schema rule, the three-error taxonomy, Bearer forwarding, and the deliberate absence of a `Content-Type` on FormData. Still untested: the lockout path on password sign-in, which cannot be reached over HTTP because `MAX_FAILED_ATTEMPTS` and the route's throttle are both 10. The magic-link sign-up / sign-in / verify / demo flows have been run against **local** Mongo (Resend unconfigured in dev, so the magic link is logged to the api console). Email + password sign-in has not been run in a browser, and nothing has run against live Resend or Gemini — treat those as unverified.
