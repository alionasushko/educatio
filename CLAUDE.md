...
`apps/web` is UI. `apps/api` is everything else. Web is forbidden from importing the api-only packages — `mongoose`/`mongodb`, `@liveblocks/node`, `@ai-sdk/anthropic`, `ai`, `@vercel/blob`, `resend`, plus the unused `next-auth`/`@auth/mongodb-adapter`. The full list is enforced at lint time by `no-restricted-imports` in [apps/web/eslint.config.mjs](apps/web/eslint.config.mjs), and those packages live only in api's workspace. If a web route needs data, add the endpoint to api — don't paper over a missing endpoint by reaching for a DB client.

### Auth flow

Nest owns the identity surface:

1. Web posts the sign-up / sign-in form to `apps/api` (`POST /auth/signup` or `/auth/signin`).
2. Api creates the user (signup only), mints a one-time token, sends the magic-link email via Resend.
3. The magic-link URL hits `apps/web/src/app/auth/callback/route.ts`, which proxies to `POST /auth/callback` and sets an `educatio_session` httpOnly cookie from the returned JWT.
