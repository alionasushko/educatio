...
- **`apps/web`** — Next 16 / React 19 / Tailwind v4 / shadcn `base-nova`. **UI only** — no DB, AI, email, or Liveblocks-server code (the boundary rule is now real, not aspirational). Has the marketing landing (`/`), the magic-link plumbing (`auth/callback` + `auth/signout` route handlers), the `proxy.ts` gate, the typed `api-client`, and session helpers. The tutor sign-up (`/sign-up`), sign-in (`/sign-in`), email-verify (`/verify`), and post-verification set-password (`/set-password`) screens are built (on shared `Input`/`Card`/`AuthLayout` primitives), including email+password sign-in alongside magic-link and a flag-gated one-click demo login. The `/dashboard` is built — lesson list with search, status filters and pagination, create and delete, and a skeleton for the cold load — as are the route boundaries (`error.tsx`, `global-error.tsx`, `not-found.tsx`). The lesson canvas (`/lesson/[id]`), student join (`/join/[code]`), the summary page with its exports (`/lesson/[id]/summary`), and `/settings` are built. Every screen in `docs/SPEC.md` §Features now exists.
- **`apps/api`** — NestJS + Fastify. Owns data (Mongoose), auth (Resend magic-link → JWT), AI (Google Gemini), Liveblocks token issuance, blob uploads, email. **All endpoints are implemented**: `auth/*`, `sessions/student`, `lessons` CRUD, `lessons/:id/snapshot`, `lessons/:id/summary`, `liveblocks/auth`, `upload`.
...
> **Status.** Every screen in `docs/SPEC.md` §Features is built, and every api endpoint is implemented. The web→api request layer is finished and hardened — one `server-only` seam, a response schema required on every call, one `ActionResult` shape for every Server Action, and route boundaries for errors and 404s (see `docs/ARCHITECTURE.md` §Request layer for the rules and the rejected alternatives). The app is **deployed** — web on Vercel at https://educatio-web.vercel.app, api on Google Cloud Run, scale-to-zero (see `docs/ARCHITECTURE.md` §Deployment for the flags, the `TRUST_PROXY` value and what was rejected). Lighthouse on the deployed site: 92 on the landing, 98–99 on the dashboard with the api warm and 87 when it cold-starts. What remains is one magic-link email actually delivered by Resend, and wider test coverage. A domain, `SENTRY_DSN` and transactional email are optional rather than blocking: `ENABLE_DEMO_LOGIN` is the entry path, so a visitor needs no mailbox, and without `RESEND_API_KEY`/`EMAIL_FROM` a real sign-up still answers `503 service_unavailable` by design. A whole-app security review ran before deploy; confirmed findings are fixed and the deliberate deferrals, each with the condition that reopens it, are in `docs/SECURITY.md`.
>
> **Verification status.** Everything compiles, builds, and passes `npm run check`. The api's responses are pinned to the shared contract by `apps/api/test/api-contract.spec.ts`. `apps/web` has Vitest (154 tests) and Playwright (57 tests across 7 specs, chromium + webkit) suites covering the canvas, the join flow, the summary, account settings and session-cookie lifetimes against **live Liveblocks**, live Vercel Blob and local Mongo. `proxy.ts`'s kind gate is now covered directly by `apps/web/src/__tests__/proxy.test.ts` — forged and expired tokens, a student turned back from every path that is not their own room, and an encoded-id bypass attempt. `api-client.ts` is covered too — the response-schema rule, the three-error taxonomy, Bearer forwarding, and the deliberate absence of a `Content-Type` on FormData. Still untested: the lockout path on password sign-in, which cannot be reached over HTTP because `MAX_FAILED_ATTEMPTS` and the route's throttle are both 10. The magic-link sign-up / sign-in / verify / demo flows have been run against **local** Mongo (Resend unconfigured in dev, so the magic link is logged to the api console). On the deployed stack the demo entry, email + password sign-in, the dashboard, the canvas against live Liveblocks, summary generation against live Gemini and the PDF and text exports have each been run in a browser. Sign-up and magic-link sign-in were walked end to end using the link from the api log rather than a delivered email, so only Resend's delivery is unproven.
...
`apps/web` is UI. `apps/api` is everything else. Web is forbidden from importing the api-only packages — `mongoose`/`mongodb`, `@liveblocks/node`, `@ai-sdk/google`, `ai`, `@vercel/blob`, `resend`, `bcrypt`, plus `next-auth`/`@auth/mongodb-adapter`, which are not installed anywhere and are listed so they cannot arrive unnoticed. The full list is enforced at lint time by `no-restricted-imports` in [apps/web/eslint.config.mjs](apps/web/eslint.config.mjs), and those packages live only in api's workspace. If a web route needs data, add the endpoint to api — don't paper over a missing endpoint by reaching for a DB client.
...
4. `apps/web/src/proxy.ts` (the session gate) verifies that JWT's signature (HS256, `AUTH_JWT_SECRET`) on every request to `/dashboard`, `/lesson/*`, `/settings`. No DB calls, no api calls — just signature check.
...
The Next file convention previously called `middleware.ts` is now `proxy.ts`. [apps/web/src/proxy.ts](apps/web/src/proxy.ts) verifies the `educatio_session` JWT (HS256 via `jose`) on `/dashboard`, `/lesson/*`, `/settings` — no DB or api calls. It runs on the **Node.js runtime**: a Next 16 proxy file cannot opt into Edge, and setting the `runtime` config option in one throws. Before editing it, read `node_modules/next/dist/docs/01-app/01-getting-started/16-proxy.md` — the file convention, matcher syntax, and runtime contract differ from middleware-era patterns.

## Constraints & policies

**Security — MUST follow:**

- Secrets live in `apps/api` only — `GOOGLE_GENERATIVE_AI_API_KEY`, `AUTH_JWT_SECRET`, `MONGODB_URI`, `RESEND_API_KEY`, `LIVEBLOCKS_SECRET_KEY`, `BLOB_READ_WRITE_TOKEN`. The web bundle may contain only `NEXT_PUBLIC_*` values (e.g. the Liveblocks _public_ key). The boundary rule enforces this — never import an api-only package into web to reach a secret.
