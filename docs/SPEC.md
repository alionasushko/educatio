...
- `POST /auth/callback` — body `{ token }`. Looks up `tokenHash`, validates expiry, marks `User.emailVerified=now`, deletes the token, mints a session JWT (HS256, payload `{ kind: 'tutor', sub: userId, email, iat, exp: +30d }`), returns `{ sessionJwt }`.
