...
- `/set-password`: authenticated; shown after magic-link verification (the callback's default landing when there's no stashed deep-link). Sets/replaces the password via `POST {EDUCATIO_API_URL}/auth/password` (authenticated — the only way a password is ever set). "Skip for now" → `/dashboard`. Reached from `/settings` as `?next=/settings`, which it returns to on success; it stays a page of its own because it is also where the magic-link callback lands, which a settings-only form could not serve.
- `auth/callback/route.ts`: receives the magic-link URL (`?token=…`) → calls `POST {EDUCATIO_API_URL}/auth/callback { token, binding }` → sets `educatio_session` (httpOnly, `SameSite=Lax`, `Secure` in prod, path `/`) and redirects to the stashed destination or `/set-password`. The **binding** is the login-CSRF defence: requesting a link mints a secret, stored hashed beside the link and dropped in the requesting browser as `educatio_link_binding`. A link opened anywhere else is refused — so an attacker cannot sign a victim into the attacker's account by sending them a link. It is part of the api's atomic match, so a refusal never consumes the token, which also makes mail scanners following the URL harmless. The cost is deliberate: **a link only works on the device that asked for it**. No token or no binding → `/sign-in?error=invalid-token|wrong-device`.
- `proxy.ts`: Node runtime (a Next 16 proxy file cannot opt into Edge — setting `runtime` throws), verifies the `educatio_session` JWT (HS256, `AUTH_JWT_SECRET`) on every request to `/dashboard`, `/lesson/*`, `/settings`, `/set-password`. No DB calls, no api calls. Unauthenticated → redirect to `/sign-in?callbackUrl=…`. It also checks the token's `kind`: a student may reach only `/lesson/<their own lessonId>` plus an allowlist of subpaths under it (currently `summary`); anywhere else in the matcher redirects them to their own lesson room.
- Sign-out: web clears the cookie, calls `POST {EDUCATIO_API_URL}/auth/signout`, redirects to `/`. The api bumps the tutor's `tokenVersion`, which is stamped into every session JWT and checked by `JwtAuthGuard` — so a token that has signed out stops working immediately instead of staying valid for the rest of its 30 days. Student sessions carry no user and are unaffected. `proxy.ts` still only checks the signature, so a revoked session can reach a page shell; every api call it makes then answers `401 session_expired`.
...
- `POST /auth/callback` — body `{ token, binding }`. Atomically consumes the link (`findOneAndUpdate` matching an unused, unexpired `tokenHash` **and** the `bindingHash` of the requesting browser's cookie, setting `usedAt` — so it can't be replayed under concurrency, and a wrong binding never consumes the token), marks `User.emailVerified=now`, mints a session JWT (HS256, payload `{ kind: 'tutor', sub: userId, email, iat, exp: +30d }`), returns `{ sessionJwt }`.
- `POST /auth/signout` — **authenticated**. Bumps the tutor's `tokenVersion`, which is stamped into every session JWT and checked by `JwtAuthGuard`, so every session for that account dies at once rather than staying valid for its remaining 30 days. The shared demo account is exempt: revoking there would sign out every other visitor exploring at the same time. Returns `{ ok: true }`.
- `GET /auth/me` — auth required; returns `{ user: { id, email, name, image?, teaches?, hasPassword, isDemo } }` (`hasPassword` drives the set-vs-change label and the hash itself is never returned; `isDemo` lets `/settings` present itself read-only instead of offering controls that 403).
- `PATCH /auth/me` — **authenticated**; body `{ name }` (1–120 chars, trimmed). Renames the caller and returns the same `{ user }` shape. The demo account is rejected. Throttled 20/min.
- `DELETE /auth/me` — **authenticated**. Deletes the caller's account and everything under it: snapshots, lessons, their Liveblocks rooms (best effort), magic links, then the user. The demo account is rejected. Returns `{ ok: true }`. Throttled 5/min.
...
| `/auth/callback`        | POST   | none                                             | body `{ token, binding }` → `{ sessionJwt }`                                                   |
| `/auth/signout`         | POST   | tutor                                            | → `{ ok: true }`                                                                               |
| `/auth/me`              | GET    | tutor                                            | → `{ user }`                                                                                   |
| `/auth/me`              | PATCH  | tutor                                            | body `{ name }` → `{ user }`                                                                   |
| `/auth/me`              | DELETE | tutor                                            | → `{ ok: true }` (account + lessons + snapshots + rooms)                                       |
