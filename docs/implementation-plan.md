...
- [x] **Project setup** — monorepo + workspace tooling. Sentry is wired in `apps/web` through `instrumentation.ts` (server and edge, plus `onRequestError`) and `instrumentation-client.ts`, both gated on a DSN so an unset environment leaves it uninitialised. `apps/api` reports through `AllExceptionsFilter`, its single exit for errors — genuine faults and deliberate 5xx only, never a 4xx the caller caused. `instrument.ts` runs before Nest boots, so it reads `SENTRY_DSN` from the real environment rather than a `.env` file.
