...
- **Cross-cutting remaining:** the replay toggle (pan/zoom over the final snapshot) — plus Sentry on both apps, CI (none), the `/settings` screen `proxy.ts` already gates, and the deferred auth hardening under Authentication.
