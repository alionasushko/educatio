...
- [ ] **Lesson canvas** — **the next stage.** Api endpoints built (`/lessons/:id`, `/lessons/:id/snapshot`, `/liveblocks/auth`); `konva`, `react-konva`, `@liveblocks/{client,react}` already in web's deps; `/lesson/[lessonId]` is a placeholder page. Canvas UI, toolbar, presence, snapshot loop not built. Its plumbing is now in place: `GET /lessons/:id/snapshot` exposes the latest snapshot for cold-load and replay, and `apps/web/src/app/liveblocks-auth/route.ts` is the `authEndpoint` the client needs, since the session cookie is httpOnly and api is another origin. What remains unbuilt: **nothing writes snapshots** — `POST /lessons/:id/snapshot` still has no caller, which is also why the AI summary has nothing to read. Note this is the first feature that cannot be exercised locally: `LIVEBLOCKS_SECRET_KEY` is optional in the env schema and api answers 503 without it. See `docs/SPEC.md` §Lesson canvas.
