...
- [x] **Project setup** — monorepo + workspace tooling. Sentry is wired in `apps/web` through `instrumentation.ts` (server and edge, plus `onRequestError`) and `instrumentation-client.ts`, both gated on a DSN so an unset environment leaves it uninitialised. `apps/api` has no Sentry dependency yet.
...
- **Cross-cutting remaining:** Sentry in `apps/api`, the `/settings` screen `proxy.ts` already gates, and the deferred auth hardening under Authentication.
