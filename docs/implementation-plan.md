...
│   │   │   │   ├── session.ts               # cookie/JWT session helpers        [built]
│   │   │   │   ├── session-server.ts        # getCurrentSession via next/headers [built]
│   │   │   │   ├── security-headers.ts      # CSP + the security header set      [built]
│   │   │   │   ├── pdf/SummaryPDF.tsx        #                                  [not built]
│   │   │   │   └── utils.ts
│   │   │   └── proxy.ts                     # Node: verifies session JWT        [built]
│   │   ├── tsconfig.json
│   │   └── package.json
...
- [x] **Project setup** — monorepo + workspace tooling. Sentry is wired in `apps/web` through `instrumentation.ts` (one `register`, plus `onRequestError`) and `instrumentation-client.ts`, both gated on a DSN so an unset environment leaves it uninitialised. `apps/api` reports through `AllExceptionsFilter`, its single exit for errors — genuine faults and deliberate 5xx only, never a 4xx the caller caused. `instrument.ts` runs before Nest boots, so it reads `SENTRY_DSN` from the real environment rather than a `.env` file.
- [x] **Marketing landing** — see `docs/SPEC.md` §Marketing landing. Lighthouse against the deployed site: performance 92, accessibility 100, best practices 100, SEO 100 (FCP 0.9 s, LCP 3.3 s, TBT 20 ms, CLS 0).
- [x] **Authentication** — api endpoints built (`/auth/*` incl. flag-gated `/auth/demo`, authenticated `/auth/password`, `JwtAuthGuard`, `proxy.ts`, `auth/callback`, `auth/signout`). Email + password sign-in added alongside magic-link (password set only post-verification via `/auth/password`; per-account lockout; client-IP forwarding for throttling). Web screens: `/sign-up`, `/sign-in`, `/verify`, and `/set-password` built (+ shared `Input`/`Card`/`AuthLayout` primitives, server actions, one-click demo login, stale-tab redirect on `/verify`). **Follow-ups:** (1) `/set-password` stays its own page, reached from `/settings` with `?next=/settings` so the reader comes back — it is also the post-verification landing, which a settings-only form could not serve; (2) magic-link login-CSRF is closed by binding each link to the browser that requested it — which means a link no longer works across devices, a cost taken deliberately over adding a confirmation click. Session revocation is done: `/auth/signout` bumps `tokenVersion` and the guard rejects tokens minted before it — except for the shared demo account, where revoking would sign every other visitor out. The kind gate is covered by `apps/web/src/__tests__/proxy.test.ts`. See `docs/SPEC.md` §Authentication.
- [x] **Tutor dashboard** — api ready (`GET /lessons`). Web page built: lesson list (card + row), search, status filters, pagination, create, delete, empty state, cold-load skeleton, shared `DashboardLayout` + sidebar. Row click uses `lessonHref`, which sends **ended** lessons to `/lesson/:id/summary`. The status column shows Active or Ended only — `scheduled` was removed along with the duration column, because nothing ever set a lesson live. Remaining: tests. See `docs/SPEC.md` §Tutor dashboard.
...
- **Verified:** `npm run check` passes (format, lint, typecheck, test) and production builds pass for all three workspaces. `apps/api`'s responses are pinned to the shared contract by `test/api-contract.spec.ts`, which boots the real Nest app against an ephemeral mongod. The magic-link sign-up / sign-in / verify / demo flows have been run against **local** Mongo. On the deployed stack — web on Vercel at https://educatio-web.vercel.app, api on Cloud Run — the demo entry, email + password sign-in, the dashboard, lesson creation and sharing, the canvas against live Liveblocks, ending a lesson, summary generation against live Gemini, and the PDF and text exports have each been exercised in a browser.
- **Not verified:** Resend's actual delivery. Sign-up and magic-link sign-in have been walked end to end using the link from the api log rather than a delivered email, so token issue, the `auth/callback` hop, `emailVerified` and the session cookie are all proven — what is untested is that an email leaves Resend and arrives. `RESEND_API_KEY` and `EMAIL_FROM` are set on Cloud Run. `apps/web` has a Vitest suite (154 tests) and a Playwright suite (57 tests across 7 specs, chromium + webkit) covering the canvas, the join flow and session-cookie lifetimes against **live Liveblocks** and local Mongo. `proxy.ts`'s kind gate is covered directly by `apps/web/src/__tests__/proxy.test.ts`: no session, an unreadable token, a token signed with another secret, an expired token, a student carrying no lesson, a tutor allowed everywhere the matcher reaches, a student allowed only into their own room and its summary, eight paths a student is turned back from, and an encoded lesson id. `api-client.ts` has its own suite covering the response-schema rule, the three-error taxonomy, Bearer forwarding and the deliberate absence of a `Content-Type` on FormData. The production content-security-policy is pinned by `lib/__tests__/security-headers.test.ts`, which asserts the whole header set by value — the e2e suite runs against `npm run dev`, so it only ever sees the dev policy.
- **Cross-cutting remaining:** one magic-link email actually delivered by Resend. The deploy itself is done (Vercel for web, Cloud Run scale-to-zero for api — see `docs/ARCHITECTURE.md` §Deployment). A domain stays _optional_: `ENABLE_DEMO_LOGIN` is the entry path, so a visitor needs no mailbox — and were `RESEND_API_KEY` ever removed, a real sign-up answers `503 service_unavailable` by design rather than failing silently. `SENTRY_DSN` stays optional.
...
- Lighthouse score >90 on landing page and dashboard — **met on the landing** (92) and **met on the dashboard while the api is warm** (98–99 across repeat runs). A cold Cloud Run instance drops the dashboard to 87: LCP goes to 4.0 s, and 83% of that is render delay on the `Lessons` heading, which waits on the lesson fetch. Accessibility 100, best practices 96, SEO 100 throughout. This is the scale-to-zero tradeoff, not a front-end defect — setting a minimum instance would fix it and cost money.
- First Contentful Paint <1.5s on landing page — met (0.9 s measured)
