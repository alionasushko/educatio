...
**`'wasm-unsafe-eval'` in the production `script-src`.** Required by the PDF export: `@react-pdf/renderer` reaches `yoga-layout`, which compiles a base64-embedded WebAssembly binary at runtime, and without the directive the browser refuses it — the download failed in production while working in dev, where `'unsafe-eval'` already covered it. Accepted because it is the narrow directive: it permits WebAssembly compilation and still forbids `eval` and `new Function`, so production stays stricter than dev, and nothing in the react-pdf or yoga tree calls either. It is orthogonal to the `'unsafe-inline'` above rather than implied by it — that keyword governs inline script execution, not string compilation. Browsers older than Chrome 97 / Firefox 102 / Safari 16 ignore the token and still refuse the export; those users keep the text and email routes. Revisit if the summary PDF moves to the api, which would remove the need entirely.
